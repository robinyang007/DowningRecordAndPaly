...
//文件的存放路径 不包含 MD5加密的文件名
#define HSFileFullpathMD5(url) [HSCachesDirectory stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.%@",HSFileName(url),[url pathExtension]]]

// 文件的已下载长度
#define HSDownloadLength(url) [[[NSFileManager defaultManager] attributesOfItemAtPath:HSFileFullpathMD5(url) error:nil][NSFileSize] integerValue]
...
//    NSOutputStream *stream = [NSOutputStream outputStreamToFileAtPath:HSFileFullpath(url) append:YES];
    NSString * filePath = HSFileFullpathMD5(url);
...
{
    NSFileManager *fileManager = [NSFileManager defaultManager];
    if ([fileManager fileExistsAtPath:HSFileFullpathMD5(url)]) {

        // 删除沙盒中的资源
        [fileManager removeItemAtPath:HSFileFullpathMD5(url) error:nil];
        // 删除任务
